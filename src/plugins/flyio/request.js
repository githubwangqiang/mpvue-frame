...
const handleRequest = (url = '', data = {}) => {
  let _url = API[url] || ''
  let matchResult = _url.match(/{.*?}/)
  if (matchResult == '{activityID}') { // eslint-disable-line
    _url = _url.replace(/{.*?}/, data['activityID'])
  }
  if (matchResult == '{userWordBookID}') { // eslint-disable-line
    _url = _url.replace(/{.*?}/, data['userWordBookID'])
  }
  ...
  return (flyConfig = {}, defaultTipConfig = {}) => {
    ...
    return flyio.then(res => {
      ...
      // console.log(err, '1错误的err---request')
      if (err.status === 460) {
        wx.setStorageSync('AccessToken', '')
        console.log('这里是token过期在首页的token的校验')
        wx.reLaunch({
          url: '/pages/home/index?isLand=nologin'
        })
      } else
      if (err.status === 409 || err.status === 450 || err.status === 461 || err.status === 414) {
        let errodata = {
          'status': err.status,
